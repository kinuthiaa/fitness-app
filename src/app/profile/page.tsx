...
import { AppleIcon, CalendarIcon, Dumbbell } from 'lucide-react';
...
const Profilepage = () => {
  ...
  const allPlans = useQuery(api.plans.getUserPlans, { userId: userId ?? "" });
  const [selectedPlanId, setSelectedPlanId] = useState<Id<"plans"> | null>(null);

  const activePlan = allPlans?.find(plan => plan.isActive);
  const currPlan = selectedPlanId ? allPlans?.find(plan => plan._id === selectedPlanId) : activePlan;

  // Don't render anything while user data is loading
  if (!isLoaded) {
              ...
              <div className="relative backdrop-blur-sm border border-border rounded-lg p-6">
                ...
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="text-lg font-bold">
                    PLAN: <span className="text-primary">{currPlan.name}</span>
                  </h3>
                </div>
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center gap-2 mb-4">
                        ...
                        <span className="font-mono text-sm text-muted-foreground">
                          SCHEDULE: {currPlan.workoutPlan.schedule.join(", ")}
                        </span>
                      </div>

                      <Accordion type="multiple" className="space-y-4">
                        {currPlan.workoutPlan.exercises.map((exerciseDay, index) => (
                          <AccordionItem
                            key={index}
                    ...
                    <div className="space-y-4">
                      <div className="flex justify-between items-center mb-4">
                        ...
                        <div className="font-mono text-xl text-primary">
                          {currPlan.dietPlan.dailyCalories} KCAL
                        </div>
                      </div>
                      ...
                      <div className="space-y-4">
                        {currPlan.dietPlan.meals.map((meal, index) => (
                          <div
                            key={index}
